...
<body>
    ...
    <section id="lewy">
        ...
        if(mysqli_num_rows($result)>0){
            while($row = mysqli_fetch_assoc($result)){
                echo "<li>".$row["nazwa"]."<span class = 'list_punktow'>".$row["punkty"]."</span></li>";

            }
        }else{
            echo "Baza nie zwrocila wierszy";
        }
        echo "</ul>";
        mysqli_close($conn);
        ?>
    ...
    <footer>
        ...
        if(!empty($_POST["id"])){
            ...
            if(mysqli_num_rows($result)> 0) {
                while($row = mysqli_fetch_assoc($result)){
                    echo "<h2>".$row["nazwa"].",".$row["punkty"]."punktow,".$row["cena"]."zł </h2>";
